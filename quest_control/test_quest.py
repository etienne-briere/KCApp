...
class TestPariteAvecLaTablette(unittest.TestCase):
    """
    Le pilotage existe en deux implémentations : Python sur le poste, Kotlin
    sur la tablette (`quest_android/`). Elles doivent donner **les mêmes
    commandes** au casque.

    Ce n'est pas une exigence esthétique. Un incident survenu pendant une
    séance pilotée depuis la tablette doit pouvoir être reproduit depuis
    l'ordinateur pour être compris — et un correctif apporté d'un côté doit
    valoir pour l'autre. Deux jeux de commandes qui divergent, c'est deux
    outils à déboguer séparément sur un même appareil.
    """

    RACINE = Path(__file__).resolve().parents[1]
    PYTHON = RACINE / "quest_control" / "quest.py"
    KOTLIN = (RACINE / "quest_android" / "src" / "main" / "java" / "fr" /
              "univrennes2" / "apex" / "casque" / "CasqueQuest.kt")

    #: Commandes présentes côté Python seulement, et pourquoi c'est légitime.
    #: Toute autre divergence doit faire échouer le test.
    ABSENCES_ADMISES = {
        "ip route": "découverte de l'adresse IP du casque : sur la tablette, "
                    "elle est saisie une fois par l'utilisateur",
        "ip addr show wlan0": "même raison",
    }

    @classmethod
    def setUpClass(cls):
        if not cls.KOTLIN.exists():
            raise unittest.SkipTest(f"{cls.KOTLIN} absent")

    @staticmethod
    def _sans_interpolation(texte: str) -> str:
        """
        Neutralise les insertions de variables avant toute extraction.

        C'est l'ordre qui compte : « ${if (active) "0" else "1"} » contient des
        guillemets, et découper le fichier sur les guillemets avant d'avoir
        retiré ces blocs tronquerait la commande au milieu. Une comparaison
        faite sur des commandes tronquées passerait sans rien prouver.
        """
        texte = re.sub(r"\$\{[^{}]*\}", "X", texte)      # Kotlin
        texte = re.sub(r"\$\w+", "X", texte)              # Kotlin, forme courte
        texte = re.sub(r"\{[^{}\n]*\}", "X", texte)      # f-strings Python
        return texte

    def _commandes(self, chemin: Path, motifs) -> set:
        texte = self._sans_interpolation(chemin.read_text(encoding="utf-8"))
        trouvees = set()
        for motif in motifs:
            for brut in re.findall(motif, texte):
                trouvees.add(" ".join(brut.split()))
        return trouvees

    def commandes_python(self) -> set:
        # Certaines commandes passent par une variable intermédiaire : les
        # ignorer laisserait croire à une parité qui n'existe pas.
        return self._commandes(self.PYTHON, (
            r'self\.shell\(\s*f?"([^"]+)"',
            r'commande\s*=\s*f?"([^"]+)"',
        ))

    def commandes_kotlin(self) -> set:
        return self._commandes(self.KOTLIN, (
            r'client\.shell\(\s*\n?\s*"([^"]+)"',
            r'commande\s*=\s*"([^"]+)"',
        ))

    def test_le_kotlin_n_invente_rien(self):
        """
        Toute commande envoyée par la tablette doit exister côté Python. Une
        commande inédite serait invérifiable : le simulateur ne saurait pas y
        répondre, et aucun test ne la couvrirait.
        """
        inedites = self.commandes_kotlin() - self.commandes_python()
        # Les appels aux services de test sont assemblés par une fonction côté
        # Python : ils échappent à l'extraction, sans diverger pour autant.
        inedites = {c for c in inedites if "content call" not in c}
        self.assertEqual(inedites, set(),
                         "commandes présentes côté tablette uniquement")

    def test_les_absences_sont_justifiees(self):
        manquantes = self.commandes_python() - self.commandes_kotlin()
        manquantes = {c for c in manquantes if "content call" not in c}
        inexpliquees = manquantes - set(self.ABSENCES_ADMISES)
        self.assertEqual(
            inexpliquees, set(),
            "commandes du poste absentes de la tablette sans justification. "
            "Si l'écart est voulu, documentez-le dans ABSENCES_ADMISES ; "
            "sinon, reportez la commande dans CasqueQuest.kt.")

    def test_le_coeur_du_pilotage_est_present(self):
        """Les gestes de séance doivent exister des deux côtés, à l'identique."""
        kotlin = self.commandes_kotlin()
        for essentielle in (
            "input keyevent KEYCODE_WAKEUP",
            "am broadcast -a com.oculus.vrpowermanager.X",
            "setprop debug.oculus.guardian_pause X",
            "getprop debug.oculus.guardian_pause",
            "monkey -p X -c android.intent.category.LAUNCHER 1",
            "am force-stop X",
            "pm clear X",
        ):
            with self.subTest(commande=essentielle):
                self.assertIn(essentielle, kotlin)
                self.assertIn(essentielle, self.commandes_python())

    def test_memes_motifs_d_echec(self):
        """
        `am` et `monkey` rendent zéro tout en écrivant leur échec sur la sortie
        standard. Les deux implémentations doivent chercher les mêmes motifs,
        sinon la tablette annoncerait un jeu lancé là où le poste verrait un
        échec.
        """
        python = self.PYTHON.read_text(encoding="utf-8")
        kotlin = self.KOTLIN.read_text(encoding="utf-8")
        for motif in ("aborted", "no activities found", "events injected: 0",
                      "does not exist"):
            with self.subTest(motif=motif):
                self.assertIn(motif, python)
                self.assertIn(motif, kotlin)

    def test_memes_constantes(self):
        kotlin = self.KOTLIN.read_text(encoding="utf-8")
        self.assertIn("com.oculus.guardian", kotlin)
        self.assertIn("content://com.oculus.rc", kotlin)

    def test_le_guide_accompagne_le_code(self):
        readme = self.RACINE / "quest_android" / "README.md"
        self.assertTrue(readme.exists())
        texte = readme.read_text(encoding="utf-8")
        for attendu in ("preparer_cle.py", "kadb", "tcpip 5555",
                        "assets/adbkey", "INTERNET"):
            with self.subTest(mention=attendu):
                self.assertIn(attendu, texte,
                              "le guide doit couvrir les prérequis "
                              "d'intégration")

    def test_la_cle_ne_peut_pas_entrer_dans_le_depot(self):
        """Une clé ADB donne un accès complet au casque : elle reste dehors."""
        ignore = (self.RACINE / "quest_android" / ".gitignore")
        self.assertTrue(ignore.exists())
        texte = ignore.read_text(encoding="utf-8")
        self.assertIn("adbkey", texte)


class TestOutilDeCle(unittest.TestCase):
    """L'outil d'extraction de clé doit rester exécutable partout."""

    OUTIL = (Path(__file__).resolve().parents[1] / "quest_android" / "outils"
             / "preparer_cle.py")

    def test_aide_disponible(self):
        env = dict(os.environ)
        env["PYTHONIOENCODING"] = "cp1252"      # console Windows
        env["NO_COLOR"] = "1"
        r = subprocess.run([sys.executable, str(self.OUTIL), "--help"],
                           capture_output=True, text=True, encoding="utf-8",
                           errors="replace", env=env, timeout=60)
        self.assertEqual(r.returncode, 0, r.stderr[-400:])
        self.assertNotIn("UnicodeEncodeError", r.stderr)

    def test_refuse_d_ecrire_dans_le_depot(self):
        """
        Le dossier du projet est suivi par Git : y déposer la clé la ferait
        entrer dans l'historique, d'où elle ne sortirait plus.
        """
        import importlib.util

        specification = importlib.util.spec_from_file_location(
            "preparer_cle", self.OUTIL)
        module = importlib.util.module_from_spec(specification)
        specification.loader.exec_module(module)

        racine = Path(__file__).resolve().parents[1]
        if module.dossier_suivi_par_git(racine):
            self.assertTrue(module.dossier_suivi_par_git(racine))
        else:
            self.skipTest("le projet n'est pas un dépôt Git sur ce poste")
