...
class HomeScreen(Screen):
    ...
    def __init__(self, scan_screen, **kwargs):
        ...
        self.start_ws_button = RoundedToggleButton(
            text_start="Adaptive Mode OFF",
            text_stop="Adaptive Mode ON",
            size_hint=(1, None),
            height=dp(55),
            font_size=25,
        ...
        # Autres boutons
        btn_box.add_widget(create_rounded_button("Sensor Connection", self.go_to_scan))
        btn_box.add_widget(create_rounded_button("Heart Rate Tracking", self.go_to_progress_FC))
        btn_box.add_widget(create_rounded_button("Game Control", self.go_to_pilotage))
        # btn_box.add_widget(create_rounded_button("Game View", self.go_to_game))
        btn_box.add_widget(create_rounded_button("Player Profil", self.go_to_profil))
    ...
    def on_toggle(self, instance):
        if instance.state == 'down':
            ...
            if not isinstance(self.scan_screen.heart_rate_data, int):
                show_info_popup(
                    title="Information",
                    message="Connect the sensor to use this feature"
                )
    ...
    def send_ip(self):
        """Envoie l'IP locale en broadcast UDP jusqu'à réception de l'IP de Unity"""
        try:
        ...
        while self.listen_UDP:
            try:
                ...
                if self.IP_Unity and elapsed > 3:
                    if self.state_label:
                        self.statut_label.text = 'waiting for game connection...'
                        self.statut_gif.set_gif('gifs/loading', interval=0.1, loop=True)
                ...
                if data.startswith(b'IP_Unity:'):
                    ...
                    self.statut_gif.set_gif('gifs/check', interval = 0.1, loop=False)
...
class ScanScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        # Crée boutons
        scan_button = create_rounded_button("Scan Devices", self.start_ble_scan, background_color=get_color_from_hex("#388E3C"))
        ...
        back_button = create_rounded_button("Back", self.go_back)
    ...
    def update_ui_after_scan(self):
        if not self.devices:
            self.device_spinner.text = "No devices found. Please try again."
            self.device_spinner.values = []
        else:
            if len(self.devices) == 1 :
                self.device_spinner.text = f"{len(self.devices)} device found"

            if len(self.devices) > 1:
                self.device_spinner.text = f"{len(self.devices)} devices found"

            self.device_spinner.values = [f"{d.name} ({d.address})" for d in self.devices]

    async def scan_ble(self):
        if self.client :
            self.device_spinner.text = "Disconnecting..."
        ...
        try :
            ...
            show_info_popup(
                ...
                message="Enable Bluetooth to use this feature"
            )
    ...
    def on_device_selected(self, spinner, text):
        if text and self.devices:
            ...
            print(f"{self.devices[0].name} ({self.devices[0].address}) == {text}")
            if selected_device:
                asyncio.create_task(self.connect_to_device(selected_device))

    async def connect_to_device(self, device):
        self.device_spinner.text = f"Connecting to {device.name}..."

        try:
            async with bleak.BleakClient(device) as self.client:
                self.device_spinner.text = f"Connection to {device.name} successful !"

                if HEART_RATE_UUID in [char.uuid for service in self.client.services for char in
                                       service.characteristics]:
                    print(f"🔊 {device.name} is broadcasting heart rate data !")
                    ...
                    self.device_spinner.text = f"{device.name} isn't broadcasting heart rate data !"
...
class ProgressScreen(Screen):
    def __init__(self, scan_screen, pilotage_screen, profil_screen, **kwargs):
        ...
        ## --- Graphique ---
        self.graph = Graph(xlabel='Time (s)', ylabel='HRmax (%)',
...
class PilotageScreen(Screen):
    ...
    def __init__(self, scan_screen, **kwargs):
        ...
        self.cube_rate_slider = create_icon_slider(
            icon_left_path="images/cube.png",
            icon_right_path="images/cubes.png",
        ...
        self.obs_button = RoundedToggleButton(
            text_start="Obstacle OFF",
            text_stop="Obstacle ON",
            size_hint=(0.3, None),
            height=70,
            font_size=25,
            pos_hint={'center_x': 0.5}  # CENTRAGE horizontal
...
class GameScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        top_title = create_icon_label_row("images/video-games.png", "Game View", font_size=30)
        top_section.add_widget(top_title)
        main_layout.add_widget(top_section)
        ...
        back_button = create_rounded_button("Back", self.go_back, size_hint=(0.8, None))
