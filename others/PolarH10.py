...
class PolarH10:
    ...
    def hr_data_conv(self, sender, data):  
        ...
        self.hr_stream_values.extend([hr])
        self.hr_stream_times.extend([time.time_ns()/1.0e9])
    ...
    # sample1, sample2,
        ...
        if data[0] == 0x02: # 0x02 -> accélération
            # Initialisation du temps de référence
            if not bool(self.acc_stream_values): # si "acc_stream_values" vide
                self.acc_stream_start_time = time.time_ns()/1.0e9 # enregistre l'h du temps actuel
            
            # Extrait et convertit le timestamp du dernier échantillon en sec
            timestamp = PolarH10.convert_to_unsigned_long(data, 1, 8)/1.0e9 # timestamp of the last sample
            # --> référence temporelle pour recalculer les timestamps des échantillons précédents
            
            # Détermination de la résolution et du taux d'échantillonnage
            frame_type = data[9] 
            resolution = (frame_type + 1) * 8 # 16 bit
            
            time_step = 0.005 # 200 Hz freq d'échantillonnage
            
            # Calcul du nombre d'échantillons disponibles
            step = math.ceil(resolution / 8.0) # Taille d'un échantillon (en octets)
            samples = data[10:] # Données brutes d'accélération
            n_samples = math.floor(len(samples)/(step*3)) # Nombre total d'échantillons
            sample_timestamp = timestamp - (n_samples-1)*time_step # Timestamp du premier échantillon
            
            # Décodage des échantillons d'accélération
            offset = 0
            MG_TO_MS2 = 0.00980665 # Facteur de conversion mg -> m/s²
            while offset < len(samples):
                x = PolarH10.convert_array_to_signed_int(samples, offset, step)*MG_TO_MS2
                offset += step
                y = PolarH10.convert_array_to_signed_int(samples, offset, step)*MG_TO_MS2
                offset += step
                z = PolarH10.convert_array_to_signed_int(samples, offset, step)*MG_TO_MS2
                offset += step
    ...
    @staticmethod
    def convert_array_to_signed_int(data, offset, length): # si nb peuvent être négatifs
        return int.from_bytes(
            bytearray(data[offset : offset + length]), byteorder="little", signed=True,
        )
    @staticmethod
    def convert_to_unsigned_long(data, offset, length):# si nb sont tjrs positifs
        return int.from_bytes(
    ...
    def get_hr_data(self):
        ...
        hr_times = np.array(self.hr_stream_times)
        hr_times = hr_times - self.acc_stream_start_time # rel to start of acc session time in unix s
        self.hr_data = {'times': hr_times, 'values': np.array(self.hr_stream_values)}
