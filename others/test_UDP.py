...
class HomeScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        self.sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # crée une socket UDP
        ...
        try:
            async for message in websocket: # ecoute les messages entrant de Unity
                ...
                if self.ID_message_recep == self.ID_message_envoi:
                    ...
                    if (not self.list_timestamp_recep
                        or self.timestamp_recep == self.list_timestamp_recep[-1]) \
                            or self.timestamp_recep < self.timestamp_envoi:
                        self.timestamp_recep = self.timestamp_envoi
                    ...
                    self.list_bandwidth.append(self.message_size_kbits/((self.timestamp_recep-self.timestamp_envoi)+0.00001))

                    print(f"nb bande passante : {len(self.list_bandwidth)}")
    ...
    async def send_data_to_clients(self, heart_rate):
        if not clients:
            ...
            #self.send_ip()
            self.send_data_UDP("IP", self.get_ip())
            print ("ENVOIE IPPP")
        ...
        if clients:
            ...
            # Envoie du message à Unity via UDP
            self.send_data_UDP("HR_test", message)

            #await asyncio.gather(*[client.send(message_ws) for client in clients])
    ...
    def send_data_UDP (self, ID, message):
        try:
            self.sock.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1) # Active le mode broadcast
            message = f"{ID}:{message}"
            message = message.encode() # en bytes
            self.sock.sendto(message, (BROADCAST_IP, UDP_PORT)) # envoie le message à l'adresse IP et au port spécifique
            print(f"📡 Message envoyé en broadcast : {message.decode()} vers {BROADCAST_IP}:{UDP_PORT}")
        except Exception as e:
            print(f"⚠️ Erreur lors de l'envoi UDP : {e}")
    ...
    def save_data_button_callback(self, instance):
        def save_to_folder(folder_path):
            ...
            base_filename = "results_perfs_UDP"
            file_ext = ".csv"
            i = 1
                ...
                if HEART_RATE_UUID in [char.uuid for service in self.client.services for char in
                    ...
                    async def callback(sender, data):
                        ...
                        if home_screen.start_ws_button.state == 'down':
                            ...
                            #home_screen.send_data_UDP("IP", home_screen.get_ip())
