    ...
    async with BleakClient(address) as client: # Connection à l'appareil (via son adresse)
        ...
        def acc_data_conv(sender, data):
            ...
            if data[0] == 0x02:  # 0x02 -> accélération
                ...
                if not bool(acc_stream_values):  # si "acc_stream_values" vide
                    acc_stream_start_time = time.time_ns() / 1.0e9  # enregistre l'h du temps actuel
        ...
        await client.start_notify(uuid, acc_data_conv) # active le descriptors correspondant à l'UUID
